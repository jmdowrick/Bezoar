classdef uieventviewermain_select_multi < TComponent
    properties (Constant)
        Type = 'hggroup'
    end
    ...
    methods
        ...
        function updateuimaps(obj)
            switch obj.Data.uiview.viewname
                case 'maps'
                    at = obj.Data.wave.waves(:, :, obj.Data.uimaps.selection);

                    [nr, nc, ~] = size(at);

                    v = reshape(permute(at, [1 2 3]), nr, []);
                    h = reshape(permute(at, [2 1 3]), nc, []);
    ...
    methods % CONSTRUCTOR
        function obj = uieventviewermain_select_multi()
